...
export function Navbar({ title, email, onLogout, hideSearch = false, compactProfile = false }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    readList();
    function onStorage(e: StorageEvent) {
      if (e.key === 'pbms.notifications' || e.key === 'pbms.notificationsCount') {
        readList();
        const raw = localStorage.getItem('pbms.notificationsCount');
        const n = raw ? parseInt(raw, 10) : 0;
        setNotifCount(Number.isFinite(n) ? n : 0);
          ...
          {compactProfile ? (
            <div className="flex items-center gap-3">
                  ...
                  if (next) {
                    ...
                    try {
                      ...
                      if (Array.isArray(list) && list.length) {
                        const updated = list.map((it: any) => ({ ...it, read: true }));
                        localStorage.setItem('pbms.notifications', JSON.stringify(updated));
                ...
                <div className="absolute right-0 top-full mt-2 w-80 max-h-72 overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-xl shadow-black/40 z-50">
                  <div className="px-3 py-2 text-sm text-slate-300">
                    <div className="flex items-center justify-between">
                      <div className="text-xs font-semibold">Thông báo</div>
                      <button
                        className="text-[11px] text-slate-400 hover:text-white"
                        onClick={() => {
                          // clear all
                          localStorage.setItem('pbms.notifications', JSON.stringify([]));
                          localStorage.setItem('pbms.notificationsCount', '0');
                          setNotifList([]);
                          setNotifCount(0);
                  ...
                  <div className="divide-y divide-white/5 max-h-56 overflow-auto">
                    {notifList && notifList.length > 0 ? (
                      notifList.slice(0, 50).map((n: any) => (
                        <div key={n.id ?? n.timestamp}
                          className={`px-4 py-3 text-sm ${n.read ? 'bg-transparent' : 'bg-slate-800/20'} hover:bg-slate-800/30 cursor-pointer`}>
                          <div className="font-semibold text-slate-100 truncate">{n.title ?? 'Thông báo'}</div>
                          {n.body && <div className="text-xs text-slate-300 mt-1 truncate">{n.body}</div>}
                          {n.timestamp && <div className="text-[11px] text-slate-400 mt-1">{new Date(n.timestamp).toLocaleString()}</div>}
                        </div>
                      ))
                    ) : (
              ...
              <div className="relative inline-block">
                <AdminUserDropdown compact onLogout={() => {
            ...
            <>
              <Button variant="secondary" size="sm" className="gap-2 rounded-full border border-white/10 bg-slate-800/70 hover:bg-slate-700/80 text-slate-300 hover:text-white shadow-sm h-9.5 px-4 transition-all relative">
