...
function validatePlate(raw: string, existingPlates: Array<{ plateNumber: string; vehicleType: 'car' | 'motorcycle' }>): PlateValidationResult {
  // Step 1: empty check
  if (!raw || raw.trim() === '') {
    return { ok: false, error: 'Please enter the license plate number.' };
  }
  ...
  if (!PLATE_REGEX.test(plate)) {
    return {
      ok: false,
      error: 'Invalid format. Valid examples: 29A-12345, 30AB-1234, 51F-99999.',
    };
  }

  // Step 3: max length sanity (already covered by regex, but extra guard)
  if (plate.length > 10) {
    return { ok: false, error: 'License plate cannot exceed 10 characters.' };
  }
...
export default function ProfilePage() {
  ...
  const handleAddPlate = () => {
    ...
    if (editPlates.length >= MAX_PLATES) {
      setPlateError(`Maximum ${MAX_PLATES} license plates per account.`);
      return;
    }
    ...
    setPlateInput('');
    setPlateSuccess(`Added license plate "${normalized}" (${vehicleType === 'car' ? 'Car' : 'Motorcycle'}) successfully!`);
  ...
  const handleSetDefaultEditPlate = async (plate: typeof editPlates[0]) => {
    // 1. Update editPlates state immediately to show on UI
    setEditPlates((prev) =>
      prev.map((p) => ({
        ...p,
    ...
    // 2. If _id exists on Backend, call setDefaultLicensePlate API to save changes
    if (plate._id) {
      try {
        await setDefaultLicensePlate(plate._id);
        setPlateSuccess(`Set license plate "${plate.plateNumber}" as default! 🌟`);
        setTimeout(() => setPlateSuccess(null), 2500);
      } catch (err) {
        setPlateError(err instanceof Error ? err.message : 'Could not set default.');
      }
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Find the edit plate that is marked as default
      const defaultPlateInEdit = editPlates.find((ep) => ep.isDefault === true);

      // If default plate is present, match it with freshPlates to retrieve the _id from MongoDB server and call setDefaultLicensePlate API
      ...
      // If default plate is present, call setDefaultLicensePlate to synchronize database
              ...
              <div>
                ...
                <p className="text-[11px] text-amber-200/80 mt-1 font-semibold leading-relaxed">
                  ...
                  {user.licensePlates.length === 0
                    ? ' No license plate linked to your account.'
                    : ''}
                  {' '}Please click "Edit Profile" to update, so the PBMS system can automatically identify your vehicle at access points.
                </p>
              </div>
            </motion.div>
          )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="space-y-1">
                ...
                <h1 className="text-3xl font-black tracking-tight text-white">
                  Personal <span className="bg-gradient-to-r from-orange-400 via-amber-400 to-blue-400 bg-clip-text text-transparent">Account Details</span>
                </h1>
              </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {editPlates.length < MAX_PLATES ? (
                      ...
                      <div className="flex items-center gap-2 rounded-xl border border-rose-500/20 bg-rose-500/5 px-4 py-2.5">
                        ...
                        <span className="text-[11px] text-rose-300 font-semibold">Maximum limit of {MAX_PLATES} plates reached. Delete one to add a new one.</span>
                      </div>
                    )}
                    ...
                    <p className="text-[9px] text-slate-500 font-semibold leading-relaxed">
                      * Valid format: 2 digits + 1-2 letters + dash + 3-5 digits. Example: <span className="font-mono text-slate-400">29A-12345</span>, <span className="font-mono text-slate-400">30AB-1234</span>. Press <kbd className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-300 font-mono text-[8px]">Enter</kbd> or click Add to save.
                    </p>
                  </div>
                )}
                    ...
                    value:
                      user.licensePlates.length > 0 ? (
                        ...
                        <span className="text-amber-400 font-bold text-xs flex items-center gap-1.5 mt-1 animate-pulse">
                          <ShieldAlert size={14} /> No license plates linked
                        </span>
                      ),
                    isCustom: true,
                  }] : []),
              ...
              {user.role === 'user' && (
                <div className="rounded-3xl bg-slate-950/40 border border-white/5 p-6 shadow-md space-y-3">
                  ...
                  <p className="text-[9px] text-slate-500 font-semibold">
                    {user.licensePlates.length === 0
                      ? 'No license plates linked.'
                      : user.licensePlates.length < MAX_PLATES
                        ? `${MAX_PLATES - user.licensePlates.length} slots remaining.`
