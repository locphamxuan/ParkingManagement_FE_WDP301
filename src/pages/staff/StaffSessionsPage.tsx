...
export function StaffSessionsPage() {
  ...
  // Shift revenue + end-of-shift report depend on BE endpoints that are still WIP on a
  // separate branch (not merged into dev yet) — show a pending notice instead of calling
  // endpoints that don't exist (previously silently showed fake $0 totals).
  return (
    <div className="space-y-5">
      <section
        className={`relative overflow-hidden rounded-2xl p-5 ${styles.headerBanner}`}
      >
        <div className="flex items-center gap-3.5">
          <div
            className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${styles.headerIcon}`}
          >
      ...
      <div className="py-16 text-center border border-dashed border-sky-100 rounded-2xl bg-sky-50/20">
        <CircleDollarSign size={32} className="mx-auto mb-2 text-slate-300" />
        <p className="text-sm text-slate-500 font-semibold">Shift revenue &amp; end-of-shift reporting is coming soon.</p>
        <p className="text-xs text-slate-400 mt-1">This feature is still being finished on the backend.</p>
      </div>
    </div>
  );
}
